<table class="table table-bordered table-striped table-hover js-dataTable dataTable mb0">
    ...
    <tbody>
        ...
        foreach ($result as $key => $row) {
            $row = (object)$row->from;
            $avatar = ((int)$row->id != 0)?"http://graph.facebook.com/".$row->id."/picture?type=square":BASE."assets/images/avatar.png";
        ?>
        <tr class="post-pending">
            <td>
                <input type="checkbox" name="id[]" id="md_checkbox_<?=$row->id?>" class="filled-in chk-col-red checkItem" value="<?="friend{-}".$row->id."{-}".$row->name."{-}0"?>">
                <label class="p0 m0" for="md_checkbox_<?=$row->id?>">&nbsp;</label>
            </td>
            <td><?=$row->id?></td>
            <td><img width="50" src="<?=$avatar?>"> <a href="https://facebook.com/<?=$row->id?>" target="_blank"><?=$row->name?></a></td>
